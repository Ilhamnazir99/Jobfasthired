    ...
    <div class="flex justify-center mt-10">
        <div class="w-full max-w-2xl bg-white shadow-md rounded-lg p-8">
            ...
            <form id="jobForm" action="{{ route('job.store') }}" method="POST" class="space-y-6">
                ...
                {{-- Job Title --}}
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Job Title <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="title" class="w-full border border-gray-300 px-4 py-2 rounded" required>
                </div>

                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Category <span class="text-red-500">*
                        </span></label>
                    <select name="category_id" class="w-full border border-gray-300 px-4 py-2 rounded">
                        <option disabled selected value="">Select a job category</option>
                ...
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Description <span
                            class="text-red-500">*</span></label>
                    <textarea name="description" rows="4" class="w-full border border-gray-300 px-4 py-2 rounded"
                        required></textarea>
                </div>

                {{-- Location --}}
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Location (City / Area) <span
                            class="text-red-500">*</span></label>
                ...
                {{-- Address --}}
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Full Address <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="address" id="address" class="w-full border border-gray-300 px-4 py-2 rounded"
                        required placeholder="123 Jalan Example, 43000 Kajang, Selangor">
                </div>
                ...
                {{-- Salary --}}
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Salary Rate <span
                            class="text-red-500">*</span></label>
                    <div class="flex items-center gap-2">
                        <input type="number" step="0.01" name="salary"
                            class="w-full border border-gray-300 px-4 py-2 rounded" required placeholder="e.g. 20">
                ...
                {{-- Schedule --}}
                <div>
                    <label class="block text-gray-700 font-semibold mb-2">Schedule <span
                            class="text-red-500">*</span></label>
                    ...
                    @foreach($days as $day)
                        ...
                        <div class="flex items-center gap-4 mb-2">
                            ...
                            <input type="checkbox" name="schedule[{{ $dayKey }}][active]" id="check-{{ $dayKey }}" value="1"
                                class="form-checkbox h-5 w-5 text-blue-600">

                            <input type="time" name="schedule[{{ $dayKey }}][start]" id="start-{{ $dayKey }}"
                                class="border border-gray-300 px-2 py-1 rounded">

                            <input type="time" name="schedule[{{ $dayKey }}][end]" id="end-{{ $dayKey }}"
                                class="border border-gray-300 px-2 py-1 rounded">
                        </div>
                    @endforeach
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="text" id="new-skill" class="flex-grow border border-gray-300 px-3 py-2 rounded"
                            placeholder="Enter skill (e.g. Laravel)">
                        <button type="button" onclick="addSkill()"
                            class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Add Skill</button>
                    </div>
                </div>

                {{-- Submit --}}
                <div class="text-right">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded shadow">Post
                        Job</button>
                </div>
            </form>
        </div>
